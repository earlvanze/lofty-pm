...
def webpack_get_pl_cutoff_config() -> dict[str, Any]:
    """Get P\u0026L cutoff configuration from Lofty via webpack injection."""
    from lofty_cdp import ensure_lofty_cdp_context
    from capture_lofty_auth_via_cdp import connect_ws
    ...
    val = resp.get("result", {}).get("result", {}).get("value", "null")
    result = json.loads(val) if isinstance(val, str) else val
    return {"endpoint": "getPlCutoffConfig", "data": result}
...
def webpack_get_pl_entry(
    ...
    """Get a P\u0026L entry for a property via webpack injection."""
    from lofty_cdp import ensure_lofty_cdp_context
    from capture_lofty_auth_via_cdp import connect_ws
    ...
    expr = f'''(async () => {{
      ...
      webpackChunklofty_investing_webapp.push([[Math.random()], {{}}, function(req){{ __req = req; }}]);
      const mod = __req(51046);
      const plEntry = await mod.b1({{propertyId: "{property_id}", year: "{y}", month: "{m}"}});
      return JSON.stringify(plEntry);
    }})()'''
...
def webpack_create_pl_entry(
    ...
    expr = f'''(async () => {{
      ...
      webpackChunklofty_investing_webapp.push([[Math.random()], {{}}, function(req){{ __req = req; }}]);
      const mod = __req(51046);
      const result = await mod.t1({{propertyId: "{property_id}", year: "{y}", month: "{m}"}});
      return JSON.stringify(result);
    }})()'''
